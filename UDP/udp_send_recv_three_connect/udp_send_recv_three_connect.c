...
#define STATION_BASE_ADDR "192.168.4.10"
#define STATION_ADDR_1    "192.168.4.11"
#define STATION_ADDR_2    "192.168.4.12"
...
// separate binaries, one for the access point and one for the station.
    ...
    if (access_point) {
        ...
        sprintf(dest_addr_str, "%s", STATION_BASE_ADDR);
...
#if ID == 1
            // Configure target IP address
            sprintf(my_addr, STATION_ADDR_1);
            sprintf(dest_addr_str, "%s", AP_ADDR);

            // Set local address, override the address assigned by DHCP
            ipaddr_aton(STATION_ADDR_1, &ip);
#elif ID == 2
            // Configure target IP address
            sprintf(my_addr, STATION_ADDR_2);
            sprintf(dest_addr_str, "%s", AP_ADDR);

            // Set local address, override the address assigned by DHCP
            ipaddr_aton(STATION_ADDR_2, &ip);
#endif
